              ...
              if (item.type === "phones") {
                return (
                  <div key={item.id} className="flex items-center gap-5 group">
                    <div className="text-black w-6 flex justify-center group-hover:scale-110 transition-transform flex-shrink-0">
                      <Phone size={20} />
                    </div>
                    <div className="flex items-center gap-4 flex-wrap">
                      {item.phones.map((phone, idx) => (
                        <React.Fragment key={phone.link}>
                          <a
                            href={phone.link}
                            className="cursor-pointer text-black text-base md:text-2xl leading-relaxed hover:underline underline-offset-4 decoration-[#c5a059]"
                          >
                            {phone.content}
                          </a>
                          {idx < item.phones.length - 1 && (
                            <span className="text-black/30 text-xl select-none">|</span>
                          )}
                        </React.Fragment>
                      ))}
                    </div>
              ...
              return (
                <div key={item.id} className="flex items-center gap-5 group">
                  ...
                  <a
                    ...
                    className="cursor-pointer text-black lowercase text-base md:text-2xl leading-relaxed hover:underline underline-offset-4 decoration-[#c5a059]"
                  >
                    {item.content}
                  </a>
                </div>
